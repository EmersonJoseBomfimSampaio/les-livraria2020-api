package com.les.livraria.api.controllers;
...
public class ClienteController {
	...
	private void atualizarDadosCliente(Cliente cliente, ClienteDto clienteDto, BindingResult result)
		...
		cliente.setQtdHorasAlmoco(null);
		clienteDto.getQtdHorasAlmoco()
				.ifPresent(qtdHorasAlmoco -> cliente.setQtdHorasAlmoco(Float.valueOf(qtdHorasAlmoco)));

		cliente.setQtdHorasTrabalhoDia(null);
		clienteDto.getQtdHorasTrabalhoDia()
				.ifPresent(qtdHorasTrabDia -> cliente.setQtdHorasTrabalhoDia(Float.valueOf(qtdHorasTrabDia)));

		cliente.setValorHora(null);
		clienteDto.getValorHora().ifPresent(valorHora -> cliente.setValorHora(new BigDecimal(valorHora)));
	...
	private ClienteDto converterClienteDto(Cliente cliente) {
		...
		cliente.getQtdHorasAlmocoOpt().ifPresent(
				qtdHorasAlmoco -> clienteDto.setQtdHorasAlmoco(Optional.of(Float.toString(qtdHorasAlmoco))));
		cliente.getQtdHorasTrabalhoDiaOpt().ifPresent(
				qtdHorasTrabDia -> clienteDto.setQtdHorasTrabalhoDia(Optional.of(Float.toString(qtdHorasTrabDia))));
		cliente.getValorHoraOpt()
				.ifPresent(valorHora -> clienteDto.setValorHora(Optional.of(valorHora.toString())));
